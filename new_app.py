...
# ── PropClear (Advanced Watermark Removal) ──────────────────────────────────
...
def load_ai_model():
    ...
    possible_paths = [
        ...
        os.path.join(BASE_DIR, 'weights', 'classifier.h5'),
        os.path.join(PARENT_DIR, 'Bulk_images', 'classifier.h5'),
        os.path.join(PARENT_DIR, 'Aiscrap', 'classifier.h5'),
    ]
    ...
    if not MODEL_PATH:
        return None, ['floorplan', 'gallery', 'masterplan'], BASE_DIR
...
if model is None:
    st.error("❌ CRITICAL: AI model (classifier.h5) not found or failed to load. Please ensure it exists in the project or sibling 'Bulk_images' folder.")
...
# ===================== PROPCLEAR ENGINES (Advanced Watermark Removal) ========
@st.cache_resource
def load_watermark_engines():
    try:
        detector = WatermarkDetector()
        inpainter = InpaintingEngine()
        post_processor = PostProcessor()
        return detector, inpainter, post_processor
    except Exception as e:
        st.error(f"Failed to load PropClear engines: {e}")
...
def resize_image(image_path, target_size, output_path, watermark_logo=None):
    try:
        ...
        # Only apply mirror effect if there is meaningful padding (>10px)
        if pad_x > 10 or pad_y > 10:
            # Scale background to fill entire canvas, then blur
            bg_scale = max(target_w / orig_w, target_h / orig_h)
            bg_w = int(orig_w * bg_scale)
            bg_h = int(orig_h * bg_scale)
            bg = img.resize((bg_w, bg_h), Image.Resampling.LANCZOS)
            # Crop background to canvas size
            bg_x = (bg_w - target_w) // 2
            bg_y = (bg_h - target_h) // 2
            bg = bg.crop((bg_x, bg_y, bg_x + target_w, bg_y + target_h))
            # Light blur for soft mirror effect
            bg = bg.filter(ImageFilter.GaussianBlur(radius=6))
            # Blend with white to get transparent/faded mirror look
            bg_arr = np.array(bg, dtype=np.float32)
            white  = np.full_like(bg_arr, 200, dtype=np.float32)
            # 0.45 = image, 0.55 = white → soft faded mirror
            blended = (bg_arr * 0.75 + white * 0.200).astype(np.uint8)
...
def process_single_image(image_path, filename, conf_thresh, qual_thresh, file_size=None,
    ...
    try:
        ...
        # ── Step 1.5: ADVANCED WATERMARK REMOVAL (PropClear) ──────────────────
        if category == "gallery":
            try:
                _detector, _inpainter, _post_processor = load_watermark_engines()
                if _detector is not None and _inpainter is not None:
                    # Load current image state
                    _pil_img = Image.open(current_file).convert("RGB")
                    _img_np = np.array(_pil_img)
                    
                    # Detection
                    _mask = _detector.detect_watermark(_img_np)
                    
                    if _mask.max() > 0:
                        # Inpainting
                        _inpainted = _inpainter.inpaint(_img_np, _mask)
                        
                        # Post-processing (feathered blend + sharpen)
                        if _post_processor is not None:
                            _final_np = _post_processor.process(
                                _inpainted, 
                                original=_img_np, 
                                mask=_mask,
                                sharpen=True,
                            ...
                            _final_np = _inpainted
                            
                        # Save result to temp file for subsequent steps (SR, Resize)
                        _tmp_clear = os.path.join(TEMP_DIR, f"temp_propclear_{uuid.uuid4().hex}.png")
                        Image.fromarray(_final_np).save(_tmp_clear, "PNG")
                        temp_files.append(_tmp_clear)
                        current_file = _tmp_clear
                        
                        if text_removal_info == "Skipped":
                            text_removal_info = "Advanced AI Removal (PropClear)"
                        else:
                            text_removal_info += " + Advanced Pass"
                    else:
                        # Detection failed or no watermark found
                        pass
            except Exception as _e:
                st.warning(f"PropClear advanced removal encountered an issue: {_e}")
